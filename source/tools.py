    ...
    for l in range(len(new_lamda)):
        ...
        for ll in range(len(old_lamda)):
            ...
            if old_lamda[ll] - new_lamda[l] < -5 * hwhm:
                ...
                flux_conv[l] += (
                    old_flux[ll]
                    * gauss_pdf(new_lamda[l], old_lamda[ll], hwhm)
                    * delta_lamda[ll]
                )
...
def convert_spectrum(
    ...
    if int_lambda is None:
        ...
        int_lambda.append(
            new_lambda[-1] + (new_lambda[-1] - new_lambda[-2]) / 2
        )
    ...
    if extrapolate_with_BB_T > 0:
        ...
        for i in range(len(new_lambda)):
            ...
            extrapol_values.append(
                np.pi
                * calc_analyt_planck_in_interval(
                    extrapolate_with_BB_T, int_lambda[i], int_lambda[i + 1]
                )
            )
        ...
        raise ValueError(
            "Error: extrapolation blackbody temperature cannot be negative."
        )
    ...
    if type == "linear":
        ...
        for i in range(len(int_lambda)):
            ...
            if int_lambda[i] < old_lambda[0]:
                ...
                interpol = old_flux[p_bot] * (
                    old_lambda[p_bot + 1] - int_lambda[i]
                ) + old_flux[p_bot + 1] * (int_lambda[i] - old_lambda[p_bot])
                ...
                for p in range(p_start, len(old_lambda)):
                    ...
                    if p == p_start:
                        if old_lambda[p_start] < int_lambda[i + 1]:
                            interpol = (
                                (int_flux[i] + old_flux[p])
                                / 2.0
                                * (old_lambda[p] - int_lambda[i])
                            )
                        ...
                        if old_lambda[p] < int_lambda[i + 1]:
                            interpol += (
                                (old_flux[p - 1] + old_flux[p])
                                / 2.0
                                * (old_lambda[p] - old_lambda[p - 1])
                            )

                        else:
                            interpol += (
                                (old_flux[p - 1] + int_flux[i + 1])
                                / 2.0
                                * (int_lambda[i + 1] - old_lambda[p - 1])
                            )
        ...
        for i in range(len(int_lambda)):
            ...
            if int_lambda[i] < old_lambda[0]:
                ...
                interpol = interpol ** (
                    1 / (old_lambda[p_bot + 1] - old_lambda[p_bot])
                )
                int_flux[i] = interpol
        ...
        for i in range(len(new_lambda)):
            ...
            if int_flux[i] == 0 or int_flux[i + 1] == 0:
                ...
                for p in range(p_start, len(old_lambda)):
                    ...
                    if p == p_start:
                        if old_lambda[p_start] < int_lambda[i + 1]:
                            interpol = (int_flux[i] * old_flux[p]) ** (
                                0.5 * (old_lambda[p] - int_lambda[i])
                            )
                        ...
                        if old_lambda[p] < int_lambda[i + 1]:
                            interpol *= (old_flux[p - 1] * old_flux[p]) ** (
                                0.5 * (old_lambda[p] - old_lambda[p - 1])
                            )
                            ...
                            interpol *= (
                                ...
                                0.5 * (int_lambda[i + 1] - old_lambda[p - 1])
                            )
                            interpol = interpol ** (
                                1 / (int_lambda[i + 1] - int_lambda[i])
                            )
                            break
...
def read_helios_spectrum(file, type="emission", star_fudge_factor=None):
    ...
    if star_fudge_factor is not None:
        ...
        if type == "star":
            ...
            spec = [
                s / star_fudge_factor for s in spec
            ]  # because star is in the denominator

    return lamda, spec


def rebin_spectrum_to_resolution(
    old_lamda, old_flux, resolution, w_unit="cm", type="linear"
):
    ...
    if type == "gaussian":
        _, rebin_flux = convolve_with_gaussian(
            old_lamda, old_flux, resolution, rebin_lamda
        )
    else:
        rebin_flux = convert_spectrum(
            old_lamda,
            old_flux,
            rebin_lamda,
            type=type,
